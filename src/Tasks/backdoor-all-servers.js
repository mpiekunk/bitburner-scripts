...
export let main = async (ns) => {
  ...
  try {
    ...
    let hackableServers = servers.filter(
      (s) => s != 'home' && !s.includes('hacknet-') && !s.includes('daemon')
    ); /*or whatever you name your purchased servers*/
    ...
    ns.print(
      `${
        hackableServers.filter((s) => myHackingLevel > ns.getServerRequiredHackingLevel(s)).length
      } servers are within our hack level (${myHackingLevel}).`
    );
    ns.print(
      `${
        hackableServers.filter((s) => myHackingLevel > ns.getServerRequiredHackingLevel(s) && ns.hasRootAccess(s))
          .length
      } rooted servers are within our hack level (${myHackingLevel})`
    );

    let toBackdoor = await getNsDataThroughFile(
      ns,
      `${JSON.stringify(hackableServers)}.filter(s => !ns.getServer(s).backdoorInstalled)`,
      '/Temp/servers-to-backdoor.txt'
    );
    ...
    ns.print(
      `${toBackdoor.filter((s) => ns.hasRootAccess(s)).length} of ${count} servers to backdoor are currently rooted.`
    );
    ...
    ns.print(
      `${toBackdoor.length} of ${count} servers to be backdoored are rooted and within our hack level (${myHackingLevel})`
    );

    for (const server of toBackdoor) {
      ns.print(`Hopping to ${server}`);
      anyConnected = true;
      for (let hop of routes[server]) ns.connect(hop);
    ...
    if (anyConnected) ns.connect('home');
  }
};
