...
const purchasedServersName = "pserv";
...
*/
    ...
    periodicScripts = [
      ...
      },
      {
        ...
        shouldRun: () =>
          4 in dictSourceFiles && getNumPortCrackers() != 5 && (getNumPortCrackers() < 3 || shouldImproveHacking()),
      },
      {
        interval: 31000,
        name: '/Tasks/ram-manager.js',
        shouldRun: () =>
          4 in dictSourceFiles &&
          dictSourceFiles[4] >= 2 &&
          !shouldReserveMoney() &&
          (getTotalNetworkUtilization() > 0.85 || xpOnly) &&
          shouldImproveHacking(),
      },
      // Buy every hacknet upgrade with up to 4h payoff if it is less than 10% of our current money or 8h if it is less than 1% of our current money
      {
        ...
        args: () => ['-c', '--max-payoff-time', '4h', '--max-spend', ns.getServerMoneyAvailable('home') * 0.1],
      },
      {
        interval: 33000,
        name: 'hacknet-upgrade-manager.js',
        shouldRun: shouldUpgradeHacknet,
        args: () => ['-c', '--max-payoff-time', '8h', '--max-spend', ns.getServerMoneyAvailable('home') * 0.01],
      },
      /**        
...
*/
      {
        ...
        requiredServer: 'home',
        shouldRun: () => !shouldReserveMoney() && shouldImproveHacking(),
      },
    ];
